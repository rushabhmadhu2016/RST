$(function(){
    ...
    return this.optional(element) || ((parseInt(element.files.length))+(parseInt($("#db_records").val())) <= 10)
}, 'Maximum 10 location images are allowed');

	$("form[name='updatebulistingform']").validate({
		rules:{
			property_name:{
				required:true,
				minlength:2,
	...
	$("form[name='updatefelistingform']").validate({
		rules:{
			property_name:{
				required:true,
				minlength:2,
